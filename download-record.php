            ...
            <div class="table-container">
                ...
                <div class="table attendance-table" id="attendaceTable">
                    <table>
                        ...
                        <tbody>
                            <?php
                            // Fetch all students enrolled in the course (from tblStudents table)
                            // This ensures we show ALL students, not just those with attendance records
                            $studentsQuery = "SELECT DISTINCT registrationNumber FROM tblStudents WHERE courseCode = :courseCode";
                            ...
                            // If no students found in tblStudents, fallback to students with attendance records
                            if (empty($studentRows)) {
                                $studentsQuery = "SELECT DISTINCT studentRegistrationNumber as registrationNumber FROM tblattendance WHERE course = :courseCode AND unit = :unitCode";
                                $stmtStudents = $pdo->prepare($studentsQuery);
                                $stmtStudents->execute([
                                    ':courseCode' => $courseCode,
                                    ':unitCode' => $unitCode,
                            ...
                            foreach ($studentRows as $row) {
                                ...
                                foreach ($distinctDatesResult as $dateRow) {
                                    ...
                                    // Display attendance status, normalize case for display
                                    if ($attendanceResult && !empty($attendanceResult['attendanceStatus'])) {
                                        $status = trim($attendanceResult['attendanceStatus']);
                                        // Normalize status to Title Case for display
                                        $statusDisplay = ucfirst(strtolower($status));
                                        // But keep "Present" and "Absent" as expected
                            ...
                            if (empty($studentRows)) {
                                echo "<tr><td colspan='" . (count($distinctDatesResult) + 1) . "'>No students found for this course and unit.</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
